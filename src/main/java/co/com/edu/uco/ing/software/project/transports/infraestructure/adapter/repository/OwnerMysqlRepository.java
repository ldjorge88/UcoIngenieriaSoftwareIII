package co.com.edu.uco.ing.software.project.transports.infraestructure.adapter.repository;
...
import co.com.edu.uco.ing.software.project.transports.infraestructure.adapter.repository.jpa.OwnerJpaRepository;
...
public class OwnerMysqlRepository implements OwnerRepository {
  ...
  public List<Owner> getList() {
    ...
    return owners.stream().
        map(ownerEntity -> {
          ...
          return Owner.ownerBuilder(ownerEntity.getDocument(), ownerEntity.getDocumentType(), ownerEntity.getFirstName());
        })
        .toList();
  }
  ...
  public Owner findById(Long id) {
    return ownerJpaRepository.findById(id)
        .map(ownerEntity -> Owner.ownerBuilder(ownerEntity.getDocument(), ownerEntity.getDocumentType(), ownerEntity.getFirstName()))
        .orElse(null);
  }

  @Override
  public Long save(Owner owner) {
    OwnerEntity ownerEntity = new OwnerEntity(owner.getDocument(), owner.getDocumentType(), owner.getFirstName());
